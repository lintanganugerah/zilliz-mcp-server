...
## 3. ✨ New Feature: Auto-Embedding

**No more manual embedding generation!** Zilliz MCP Server now supports automatic embedding generation using OpenRouter API.

### Key Benefits

- ✅ **One-step insert**: Just provide `content` field, embeddings auto-generated
- ✅ **Batch support**: Efficient batch embedding for multiple entities
- ✅ **Configurable**: Use any OpenRouter-supported embedding model
- ✅ **Backward compatible**: Still works with manual vectors
...
```python
# Before: Manual embedding required
embedding = generate_embedding("content")
insert_entities(data={"content": "...", "vector": embedding})

# After: Auto-embedding! ✨
insert_entities(data={"content": "..."})  # Vector auto-generated!
```

📖 **[Read Full Auto-Embedding Guide](AUTO_EMBEDDING_GUIDE.md)**
...
- **OpenRouter API Key** (Optional): For auto-embedding feature. Get one at [OpenRouter](https://openrouter.ai/).
...
_Note: Make sure to replace `/path/to/your/zilliz-mcp-server` with the actual absolute path to the project directory._
